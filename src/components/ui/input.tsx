...
const Input = React.forwardRef<HTMLInputElement, InputProps>(
  ({ className, type, left, right, ...props }, ref) => {
    ...
    return (
      <div
        className={cn(
          "flex h-12 w-full overflow-hidden rounded-full border border-neutral-600 bg-white text-neutral-600 transition hover:bg-neutral-50 has-[input:disabled]:pointer-events-none has-[input:disabled]:bg-neutral-800 has-[input:disabled]:text-neutral-400 has-[input:disabled]:opacity-60 has-[input:focus-visible]:ring has-[input:focus-visible]:ring-neutral-400 [&_input]:placeholder:text-neutral-400 [&_svg]:size-5 [&_svg]:text-neutral-500",
          className,
        )}
        onPointerDown={(event) => {
